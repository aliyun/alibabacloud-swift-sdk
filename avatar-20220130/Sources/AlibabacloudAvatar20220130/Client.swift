...
open class Client : AlibabacloudOpenApi.Client {
    ...
    @available(macOS 10.15, iOS 13, tvOS 13, watchOS 6, *)
    public func submitTextToSignVideoTaskWithOptions(_ tmpReq: SubmitTextToSignVideoTaskRequest, _ runtime: TeaUtils.RuntimeOptions) async throws -> SubmitTextToSignVideoTaskResponse {
        try TeaUtils.Client.validateModel(tmpReq)
        var request: SubmitTextToSignVideoTaskShrinkRequest = SubmitTextToSignVideoTaskShrinkRequest([:])
        AlibabaCloudOpenApiUtil.Client.convert(tmpReq, request)
        if (!TeaUtils.Client.isUnset(tmpReq.app)) {
            request.appShrink = AlibabaCloudOpenApiUtil.Client.arrayToStringWithSpecifiedStyle(tmpReq.app, "App", "json")
        }
        ...
        var tmp: [String: Any] = try await callApi(params as! AlibabacloudOpenApi.Params, req as! AlibabacloudOpenApi.OpenApiRequest, runtime as! TeaUtils.RuntimeOptions)
        return Tea.TeaConverter.fromMap(SubmitTextToSignVideoTaskResponse(), tmp)
    }

    @available(macOS 10.15, iOS 13, tvOS 13, watchOS 6, *)
    public func submitTextToSignVideoTask(_ request: SubmitTextToSignVideoTaskRequest) async throws -> SubmitTextToSignVideoTaskResponse {
        var runtime: TeaUtils.RuntimeOptions = TeaUtils.RuntimeOptions([:])
        return try await submitTextToSignVideoTaskWithOptions(request as! SubmitTextToSignVideoTaskRequest, runtime as! TeaUtils.RuntimeOptions)
    }
}
