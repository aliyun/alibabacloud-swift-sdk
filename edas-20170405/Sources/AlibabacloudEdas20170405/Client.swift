...
open class Client : AlibabacloudOpenApi.Client {
    ...
    public func edasProduceWithOptions(_ request: EdasProduceRequest, _ runtime: TeaUtils.RuntimeOptions) async throws -> EdasProduceResponse {
        try TeaUtils.Client.validateModel(request)
        var query: [String: Any] = [:]
        if (!TeaUtils.Client.isUnset(request.data)) {
            query["data"] = request.data ?? "";
        }
        if (!TeaUtils.Client.isUnset(request.sourceFlag)) {
        ...
        return Tea.TeaConverter.fromMap(EdasProduceResponse(), tmp)
    }

    @available(macOS 10.15, iOS 13, tvOS 13, watchOS 6, *)
    public func edasProduce(_ request: EdasProduceRequest) async throws -> EdasProduceResponse {
        var runtime: TeaUtils.RuntimeOptions = TeaUtils.RuntimeOptions([:])
        return try await edasProduceWithOptions(request as! EdasProduceRequest, runtime as! TeaUtils.RuntimeOptions)
    }
}
